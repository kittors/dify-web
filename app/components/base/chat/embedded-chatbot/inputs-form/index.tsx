    ...
    <div className={cn('mb-6 flex flex-col items-center px-4 pt-6', isMobile && 'mb-4 pt-4')}>
      <div className={cn(
        ...
        <div className={cn(
          ...
          <div className='system-xl-semibold grow text-text-secondary'>{"财务小助手"}</div>
          {collapsed && (
            <Button className='uppercase text-text-tertiary' size='small' variant='ghost' onClick={() => setCollapsed(false)}>{"修改"}</Button>
          )}
          {!collapsed && currentConversationId && (
            <Button className='uppercase text-text-tertiary' size='small' variant='ghost' onClick={() => setCollapsed(true)}>{"关闭"}</Button>
          )}
        </div>
        {!collapsed && (
